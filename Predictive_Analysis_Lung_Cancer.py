...
# Filtered Data
filtered_df = df[
    (df["diagnosis_date"] < pd.to_datetime("2024-12-31"))
    & (df["end_treatment_date"] < pd.to_datetime("2024-12-31"))
]
...
df["diagnosis_date"] = pd.to_datetime(
    df["diagnosis_date"], errors="coerce", format="%Y-%m-%d"
)
df["end_treatment_date"] = pd.to_datetime(
    df["end_treatment_date"], errors="coerce", format="%Y-%m-%d"
)
...
df["survival_group"] = pd.cut(
    df["survival_months"], bins=bins_survival, labels=survival_labels, right=False
)
df["age_group"] = pd.cut(df["age"], bins=bins_age, labels=labels_age, right=False)
df["cholesterol_group"] = pd.cut(
    df["cholesterol_level"], bins=bins_colesterol, labels=labels_colesterol, right=False
)
...
selected_features_cols = selected_features.tolist() + [
    "survival_months",
    "bmi",
    "cholesterol_group",
]
